...
- **OCRの実行可否は`process_page()`内の`any_visible`（そのページに可視テキストが
  既にあるか）だけで判定する**（`_maybe_run_ocr()`）。回転ページ・全面画像ページ・
  スキャンページのどれであっても、可視テキストが既にある場合はOCRしない
...
- Tesseractのサブプロセスもworker.pyと同じ考え方（`start_new_session=True` +
  ...
  (`timeout_seconds`)の残り時間で頭打ちにする（1ページのOCRだけで全体の
  処理時間の予測可能性が崩れないようにするため）。
- worker.pyは子プロセスの`TMPDIR`/`TMP`/`TEMP`を自身が管理する一時ディレクトリ
  配下に固定する。ハードタイムアウトでSIGKILLされてもOCR等が作った一時ファイルは
  そのディレクトリの内側に収まるため、worker.py自身の`shutil.rmtree(tmp_dir)`で
  確実に削除できる（機密PDFのページ画像が`/tmp`直下に残り続けるのを防ぐ）。
